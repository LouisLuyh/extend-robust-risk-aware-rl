...
class Adversary():
  ...
  """
  PlotDistortion's first subgraph plots the distribution of X theta T, X phi T,
   w. density as the default label of y axis
  """
...
class Agent():
 ...
 # in problems with other sources of loss dependent on X_phi, such as inventory loss in Problem 3, sim_phi will update self.other_loss
    ...
    for i in range(outer_epochs):
      ...
      for j in range(freeze_market_iter):
        ...
        if curr_iter % plot_freq_outer == 0:
          """
          if current iteration mod out frequency == 0, plot
          out frequency = out iteration - 1
          will only plot at the end of training
          """
  ...
  def PlotHistory(self):
    ...
    plt.xlabel("Iteration", fontsize = 20)
    plt.ylabel("Transaction Cost", fontsize = 20)
    # second subplot is R[X_theta] history
    """
    'risk' decrease as Iteration increase, finally converged to minimum
    """
    plt.subplot(1, 3, 2)
    plt.plot(self.rm_theta_history)
    ...
    """
    'risk" as described in the objective function converged to minimum as Iteration increases
    """
    plt.subplot(1, 3, 3)
